...
class NewsVC: UIViewController {
   ...
   // MARK: - Properties
   fileprivate weak var newsTVC: NewsTVC!
   
   @IBOutlet weak var horizontalBar: UIView!
   @IBOutlet weak var ofInterestButton: UIButton!
   ...
   @IBAction func ofIntrestButtonTappred(_ sender: UIButton) {
      ...
      newsTVC.display = .ofInterest
   }
   
   @IBAction func updatesTapped(_ sender: UIButton) {
      animateHorizontalBar(to: sender)
      newsTVC.display = .update
      newsTVC.tableView.visibleCells.forEach { cell in
         if let updateCell = cell as? UpdateCell {
            updateCell.isSelected = false
            updateCell.isExpanded = false
         }
         else {
